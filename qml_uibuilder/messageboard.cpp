...
#include "receiver.h"
    ...
    void onload(unsigned handle, void *userData, void *data, unsigned size){
        ...
        QObject* obj=messageboard::getRootObject()->findChild<QObject*>("itemModel");
        QMetaObject::invokeMethod(obj,"loadItems",Q_ARG(QString, QString(response.c_str())));
    }
...
bool messageboard::postMessage(const QString &msg) {
        ...
        QObject* obj=messageboard::rootObject->findChild<QObject*>("bookButton");
...
void messageboard::refresh() {
        ...
        qDebug() << "refresh";
        //QString param="";
        //emscripten_async_wget2_data("/get","GET",param.toStdString().c_str(),NULL,true,&onload,&onerror,&onprogress);
    #endif
    #ifdef __ANDROID__
        EM_ASM(
            console.log('Refresh');
...
//        },obj->objectName().toStdString().c_str());//msg);
    #endif
    #ifdef __NODEJS__
        QObject* obj=messageboard::rootObject->findChild<QObject*>("button");
        QMetaObject::invokeMethod(obj,"clickReturn",Q_ARG(QString, QString(msg)));
    //        EM_ASM({
    //            console.log(UTF8ToString($0));
    //        },obj->objectName().toStdString().c_str());//msg);
